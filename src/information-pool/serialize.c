...
void* serializeNode(const p_node_t *node, uint16_t k, uint16_t current_k, void* currentPointer, void* startPointer, uint16_t* countNode, uint16_t* countEdge);
void* serializeEdge(const p_edge_t *edge, uint16_t k, uint16_t current_k, void* currentPointer, void* startPointer, uint16_t* countNode, uint16_t* countEdge);
 ...
 * node: the pointer to the p_node of the node itself
 ...
 */
void* serialize(const p_node_t *node, uint16_t k, uint16_t maxNodes, uint16_t maxEdges, uint16_t *countNode, uint16_t *countEdge)
{
	...
	*countEdge = 0;
	...
	currentPointer = serializeNode(node, k, 0, currentPointer, startPointer, countNode, countEdge);

	return startPointer;
}

/**
 * copys p_node into the buffer at the position of currentPointer and calls serializeEdge, which in turn calls serializeNode
	...
	*countNode = (*countNode)+1;
	...
	if(current_k < k)
	{
		...
		if(node->edges != NULL)
		{
			...
			currentPointer = serializeEdge(node->edges, k, current_k+1, currentPointer, startPointer, countNode, countEdge);
		}
	}
	else
	{
...
/*
 * copys p_edge into the buffer at the position of currentPointer and calls serializeNode, which in turn calls serializeEdge
...
void* serializeEdge(const p_edge_t* edge, uint16_t k, uint16_t current_k, void* currentPointer, void* startPointer, uint16_t* countNode, uint16_t* countEdge)
{
	...
	*countEdge = (*countEdge)+1;
	...
	if(edge->next != NULL)
	{
		...
		currentPointer = serializeEdge(edge->next, k, current_k, currentPointer, startPointer, countNode, countEdge);
	}

	//go into the drain and continue crawling
	((p_edge_t*)tempptr)->drain = (p_node_t*)(currentPointer-startPointer);
	currentPointer = serializeNode(edge->drain, k , current_k, currentPointer, startPointer, countNode, countEdge);
...
{
	p_node_t *this_node;
	this_node = deserializeNode(startPointer, startPointer);
...
p_node_t* deserializeNode(void* startPointer, void* currentPointer)
{
	...
	{
		currentPointer = (void*) (startPointer + (uintptr_t)(this_node->edges));
...
p_edge_t* deserializeEdge(void* startPointer, void* currentPointer)
{
	...
	{
		currentPointer = (void*) (startPointer + (uintptr_t)(this_edge->next));
		this_edge->next = deserializeEdge(startPointer, currentPointer);
	}

	//go deeper into the tree
	currentPointer = (void*) (startPointer + (uintptr_t)(this_edge->drain));
