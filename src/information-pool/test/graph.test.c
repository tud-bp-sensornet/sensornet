...
UNIT_TEST_REGISTER(simple_handleGraphTimeout, "A simple timeout test to see if an edge+drain, and everything further in the graph, gets removed");
UNIT_TEST_REGISTER(simple_handleGraphTimeout2, "A simple timeout test to see if not every edge+drain get removed");
UNIT_TEST_REGISTER(complex_handleGraphTimeout, "A complex timeout test");
...
UNIT_TEST(edgeAlreadyExists)
	{
        ...
        UNIT_TEST_ASSERT(edgeAlreadyExists(er1, graph.root));
        UNIT_TEST_ASSERT(edgeAlreadyExists(er2, graph.root));
        UNIT_TEST_ASSERT(edgeAlreadyExists(er3, graph.root));
        ...
        UNIT_TEST_ASSERT(edgeAlreadyExists(e11, n1));
        UNIT_TEST_ASSERT(edgeAlreadyExists(e12, n1));
        UNIT_TEST_ASSERT(edgeAlreadyExists(e13, n1));
        UNIT_TEST_ASSERT(edgeAlreadyExists(e14, n1));
        ...
        UNIT_TEST_ASSERT(edgeAlreadyExists(e21, n2));
        ...
        UNIT_TEST_ASSERT(edgeAlreadyExists(e31, n3));
        UNIT_TEST_ASSERT(edgeAlreadyExists(e32, n3));
        UNIT_TEST_ASSERT(edgeAlreadyExists(e33, n3));
        ...
        UNIT_TEST_ASSERT(edgeAlreadyExists(e41, n4));
        ...
        UNIT_TEST_ASSERT(edgeAlreadyExists(e51, n5));
        ...
        UNIT_TEST_ASSERT(edgeAlreadyExists(eX, nX));
...
/**
 * Test if the timeout works for a simple graph
 */
UNIT_TEST(simple_handleGraphTimeout)
	{
		p_graph_t graph;
        p_node_t *n1, *n2;
        p_edge_t *e1, *e2;
        ...
        //set the timeouts
        e1->ttl = clocktimeAsMinutes()-1;	//should be deleted
        e2->ttl = clocktimeAsMinutes()+1;	//should also be deleted since e1 gets deleted

        //delete old information from the graph
        handleGraphTimeout(&graph);
...
/**
 * Test if the timeout works for another simple graph
 */
UNIT_TEST(simple_handleGraphTimeout2)
	{
		p_graph_t graph;
        p_node_t *n1, *n2, *n3, *n4;
        p_edge_t *er1, *er2, *e11, *e12, *e21;
        ...
        //set the timeouts
        er1->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted
        er2->ttl = clocktimeAsMinutes()-1;	//should be deleted
        e11->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted
        e12->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted
        e21->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted
 ...
 * Test if the timeout works for another simple graph
 */
UNIT_TEST(complex_handleGraphTimeout)
	{
		p_graph_t graph;
        p_node_t *n1, *n2, *n3, *n4, *n5, *n6, *n7;
        p_edge_t *er1, *er2, *er3, *er4, *er5, *e11, *e12, *e13, *e21, *e22, *e23, *e31, *e32, *e33, *e41, *e51, *e61;
        ...
        n5 = new_node(&(graph.num_nodes));
        n6 = new_node(&(graph.num_nodes));
        n7 = new_node(&(graph.num_nodes));
        er1 = new_edge(&(graph.num_edges));
        er2 = new_edge(&(graph.num_edges));
        er3 = new_edge(&(graph.num_edges));
        er4 = new_edge(&(graph.num_edges));
        er5 = new_edge(&(graph.num_edges));
        e11 = new_edge(&(graph.num_edges));
        e12 = new_edge(&(graph.num_edges));
        e13 = new_edge(&(graph.num_edges));
        e21 = new_edge(&(graph.num_edges));
        e22 = new_edge(&(graph.num_edges));
        e23 = new_edge(&(graph.num_edges));
        e31 = new_edge(&(graph.num_edges));
        e32 = new_edge(&(graph.num_edges));
        e33 = new_edge(&(graph.num_edges));
        e41 = new_edge(&(graph.num_edges));
        e51 = new_edge(&(graph.num_edges));
        e61 = new_edge(&(graph.num_edges));
        ...
        //Build Graph
        //root:er1->n1, er2->n2, er3->n3, er4->n4, er5->n5
        //n1:e11->root, e12->n2, e13->n3
        //n2:e21->root, e22->n1, e23->n3
        //n3:e31->root, e32->n1, e33->n2
        //n4:e41->root
        //n5:e51->n6
        //n6:e61->n7
        graph.root->edges = er1;
        ...
        er1->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted
        er2->ttl = clocktimeAsMinutes()-1;	//should be deleted
        er3->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted
        er4->ttl = clocktimeAsMinutes()-1;	//should be deleted
        er5->ttl = clocktimeAsMinutes()-1;	//should be deleted
        e11->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted
        e12->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted
        e13->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted
        e21->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted
        e22->ttl = clocktimeAsMinutes()-1;	//should be deleted
        e23->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted
        e31->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted
        e32->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted
        e33->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted
        e41->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted, gets deleted due to node 4 getting deleted
        e51->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted, gets deleted due to node 5 getting deleted
        e61->ttl = clocktimeAsMinutes()+1;	//shouldn't be deleted, gets deleted due to node 6 getting deleted
...
void main() {
	...
	UNIT_TEST_RUN(simple_handleGraphTimeout);
	UNIT_TEST_RUN(simple_handleGraphTimeout2);
	UNIT_TEST_RUN(complex_handleGraphTimeout);
